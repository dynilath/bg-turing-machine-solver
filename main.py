...
if __name__ == "__main__":
    ...
    def print_best(ent_r: typing.List[typing.Tuple[float, typing.Tuple[int, ...], InputType]]):
        for i in range(3):
            if i >= len(ent_r):
                break
            print("Entropy: %f, Criteria: %s, Input: %s" %
                  (ent_r[i][0], ent_r[i][1], ent_r[i][2]))

    en = CriteriaEntropy(gs.activated_group_idx,
                         gs.possible_criteria_combination)

    print("Best entropy for each input:")
    print_best(en.cal_entropy_all())
    ...
    while True:
        ...
        print_best(en.cal_entropy_for_input(code))
